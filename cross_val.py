...
def kFold_linreg(x, y, degree, lin_model, lmbda=None):
    ...
    if lmbda is None:
        model = lin_model(fit_intercept = False)
    else:
    ...
    for i, (train_inds, test_inds) in enumerate(kfold.split(x_shuffled)):
        ...
        y_train = x_shuffled[train_inds]

        x_test = x_shuffled[test_inds]
        y_test = x_shuffled[test_inds]

        # Centring of the test and train data:
    ...
    # # check_scores = -cross_val_score(linreg, X, y, scoring="neg_mean_squared_error", cv=kfold)

    # x_centred = x - np.mean(x, axis = 0)
    # y_centred = y - np.mean(y)
    # X_centred = poly.fit_transform(x_centred)
    # check_scores = -cross_val_score(linreg, X_centred, y_centred, scoring = "neg_mean_squared_error", cv = kfold)
...
def main():
    ...
    # score, check = kFold_linreg(xy, z.flatten(), degree = 5)
    # print("My code:")
    # pprint(score)
    # print("Sklearn code:")
    # pprint(check)
